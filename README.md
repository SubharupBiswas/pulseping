...
* **Native PostgreSQL Connection Pooling:** Leverages `@prisma/adapter-pg` with `pg.Pool` (`src/lib/db.ts`) for high-performance TCP connection pooling and zero-downtime query execution across serverless and hot-reloading runtimes.
